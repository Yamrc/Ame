...
pub fn render(props: ShortTrackItemProps, actions: ShortTrackItemActions) -> AnyElement {
    ...
    let row_id: SharedString = format!("short-track-item-{}", props.state_id).into();
    let menu_id = format!("short-track-item-menu-{}", props.state_id);
    ...
    let row = div()
        ...
        .child(match props.cover_url.as_deref() {
            Some(url) => img(image_resize_url(url, "64y64"))
                .id(format!("cover.short.{}", props.state_id))
                .size(px(36.))
                .rounded_md()
                .overflow_hidden()
